      ...
      switch (M.args.toLowerCase()) {
         case 'normal':
          audio = await converter(media.data, 'mp3')
            break;
        case 'nightcore':
          audio = await converter(media.data, 'mp3',["-af","asetrate=44100*1.15,aresample=44100,atempo=1.15"])
          break;
        case 'bassboost':
          audio = await converter(media.data, 'mp3',["-af","bass=g=30, dynaudnorm=f=150"])
          break;
        case 'lofi':
          audio = await converter(media.data, 'mp3',["-af","aformat=sample_fmts=s16:sample_rates=22050, lowpass=f=1200, highpass=f=100, afftdn=nf=-30, aecho=0.8:0.88:60:0.36, atempo=0.85, equalizer=f=300:t=q:w=1.5:g=5, equalizer=f=1000:t=q:w=1.5:g=3"])
          break
         default:
         await Neko.sendTextMessage(M.from, "Invalid audio type. Please use --list to see the available audio types.",M)
            break;
      }
