  ...
  run: async (Neko, M) => {
    try {
      if (!M?.isQuoted && !M?.isMentioned) {
        await Neko.sendTextMessage(M.from, "Please reply or mention to the user you want to demote",M);
        return;
      }
      const user = M.isMentioned ? M.mention[0] : M.quoted.sender;
      if (M?.groupOwner === user) {
        await Neko.sendTextMessage(M.from, "This user is the group owner. You can't demote him",M);
        return;
      }
